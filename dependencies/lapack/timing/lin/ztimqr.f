      SUBROUTINE ZTIMQR( LINE, NM, MVAL, NVAL, NK, KVAL, NNB, NBVAL,
...
*  =======
*
*  ZTIMQR times the LAPACK routines to perform the QR factorization of
...
*          The values of the matrix dimension K, used in ZUNMQR.
*
*  NNB     (input) INTEGER
...
*     .. External Subroutines ..
     ...
     $                   ZGEQRF, ZLACPY, ZLATMS, ZTIMMG, ZUNGQR, ZUNMQR
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          DBLE, MAX, MIN
*     ..
*     .. Data statements ..
      DATA               SUBNAM / 'ZGEQRF', 'ZUNGQR', 'ZUNMQR' /
...
*
                  ...
                  S1 = DSECND( )
   10             CONTINUE
                  CALL ZGEQRF( M, N, A, LDA, TAU, WORK, LW, INFO )
                  S2 = DSECND( )
                  TIME = S2 - S1
                  IC = IC + 1
...
*
                  ...
                  OPS = DOPLA( 'ZGEQRF', M, N, 0, 0, NB )
                  RESLTS( INB, IM, ILDA, 1 ) = DMFLOP( OPS, TIME, INFO )
               ELSE
*
*                 If ZGEQRF was not timed, generate a matrix and factor
*                 it using ZGEQRF anyway so that the factored form of
...
*
               IF( TIMSUB( 2 ) ) THEN
*
*                 ZUNGQR:  Generate orthogonal matrix Q from the QR
*                 factorization
*
                  ...
                  S1 = DSECND( )
   30             CONTINUE
                  CALL ZUNGQR( M, MINMN, MINMN, B, LDA, TAU, WORK, LW,
     $                         INFO )
                  S2 = DSECND( )
                  TIME = S2 - S1
                  IC = IC + 1
...
*
                  TIME = ( TIME-UNTIME ) / DBLE( IC )
                  OPS = DOPLA( 'ZUNGQR', M, MINMN, MINMN, 0, NB )
...
*
   ...
   90 CONTINUE
*
*     Time ZUNMQR separately.  Here the starting matrix is M by N, and
...
*
  ...
  100    CONTINUE
*
*        ZUNMQR:  Multiply by Q stored as a product of elementary
*        transformations
*
*        Do for each pair of values (M,N):
*
...
*
                        ...
                        DO 130 ITRAN = 1, 2
                           ...
                           S1 = DSECND( )
  110                      CONTINUE
                           CALL ZUNMQR( SIDE, TRANS, M1, N1, K1, A, LDA,
...
*
                           TIME = ( TIME-UNTIME ) / DBLE( IC )
                           OPS = DOPLA( 'ZUNMQR', M1, N1, K1, ISIDE-1,
     $                           NB )
                           RESLTS( INB, IM, ILDA,
