...
*     .. External Subroutines ..
      EXTERNAL           CGEQPF, CLAIC1, CLASCL, CLASET,
     $                   CLATZM, CTRSM, CTZRQF, CUNM2R, 
     $                   SLABAD, XERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          CONJG, REAL, ABS, MAX, MIN
*     ..
*     .. Data statements ..
...
*
         OPCNT( GELSX ) = OPCNT( GELSX ) + REAL( 6*M*N )
...
*        Scale matrix norm up to SMLNUM
*
         OPCNT( GELSX ) = OPCNT( GELSX ) + REAL( 6*M*NRHS )
...
*
         IF( SMAXPR*RCOND.LE.SMINPR ) THEN
            OPCNT( GELSX ) = OPCNT( GELSX ) + REAL( RANK*6 )
            DO 20 I = 1, RANK
...
*
     ...
     $   REAL( 8*( (N-RANK)*NRHS + NRHS + (N-RANK)*NRHS )*RANK )
         TIM1 = SECOND( )
         DO 50 I = 1, RANK
...
*
     ...
     $                INFO )
      END IF
      IF( IBSCL.EQ.1 ) THEN
         OPCNT( GELSX ) = OPCNT( GELSX ) + REAL( 6*N*NRHS )
