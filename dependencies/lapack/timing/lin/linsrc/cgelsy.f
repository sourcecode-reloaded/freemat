...
*     .. Local Scalars ..
     ...
     $                   J, LWKOPT, MN, NB, NB1, NB2, NB3, NB4, TRSM,
     $                   TZRZF, UNMQR, UNMRZ
...
*     .. External Subroutines ..
      EXTERNAL           CCOPY, CGEQP3, CLAIC1, CLASCL,
     $                   CLASET, CTRSM, CTZRZF, CUNMQR,
     $                   CUNMRZ, SLABAD, XERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          REAL, ABS, MAX, MIN, CMPLX
*     ..
*     .. Data statements ..
...
*
         OPCNT( GELSY ) = OPCNT( GELSY ) + REAL( 6*M*N )
...
*        Scale matrix norm up to SMLNUM
*
         OPCNT( GELSY ) = OPCNT( GELSY ) + REAL( 6*M*NRHS )
...
*        A * P = Q * R
*
      OPCNT( GEQP3 ) = OPCNT( GEQP3 ) + SOPLA( 'CGEQPF', M, N, 0, 0, 0 )
      T1 = SECOND( )
...
*
         IF( SMAXPR*RCOND.LE.SMINPR ) THEN
            OPCNT( GELSY ) = OPCNT( GELSY ) + REAL( RANK*6 )
            DO 20 I = 1, RANK
...
*
      IF( RANK.LT.N ) THEN
         NB = ILAENV( 1, 'UNMRQ', 'LC', N, NRHS, RANK, -1 )
         OPCNT( UNMRZ ) = OPCNT( UNMRZ ) +
     $                    SOPLA( 'CUNMRQ', N, NRHS, RANK, 0, NB )
         T1 = SECOND( )
         CALL CUNMRZ( 'Left', 'Conjugate transpose', N, NRHS, RANK,
     $                N-RANK, A, LDA, WORK( MN+1 ), B, LDB,
     $                WORK( 2*MN+1 ), LWORK-2*MN, INFO )
         T2 = SECOND( )
         TIMNG( UNMRZ ) = TIMNG( UNMRZ ) + ( T2-T1 )
      END IF
*
...
*
      IF( IASCL.EQ.1 ) THEN
         OPCNT( GELSY ) = OPCNT( GELSY ) +
     $                    REAL( ( N*NRHS + RANK*RANK )*6 )
     ...
     $                INFO )
      END IF
      IF( IBSCL.EQ.1 ) THEN
         OPCNT( GELSY ) = OPCNT( GELSY ) + REAL( N*NRHS*6 )
