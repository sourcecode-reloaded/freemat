...
*
*              INCREMENT OP COUNT
               IOPS = IOPS + (FLOAT(16*(N-L)+25)*(NCU-L)+6*(N-L)+9)
               DO 210 J = LP1, NCU
...
*           IF (ZTEST .NE. TEST) GO TO 420
            IF (CABS(E(L)) .GT. EPS * TEST) GOTO 420
*
               E(L) = (0.0E0,0.0E0)
C        ......EXIT
               GO TO 440
...
*              IF (ZTEST .NE. TEST) GO TO 460
               IF (CABS(S(LS))  .GT. EPS * TEST) GOTO 460
*
                  S(LS) = (0.0E0,0.0E0)
C           ......EXIT
                  GO TO 480
...
*           INCREMENT OP COUNT
            ...
            IF (WANTV) IOPS = IOPS+FLOAT(MM1-L+1)*12*P
            IF (WANTU) IOPS = IOPS+FLOAT(MAX((MIN(MM1,N-1)-L+1),0))*12*N
            DO 600 K = L, MM1
