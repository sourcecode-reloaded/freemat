      SUBROUTINE ZLAQZH( ILQ, ILZ, N, ILO, IHI, A, LDA, B, LDB, Q, LDQ,
     $                   Z, LDZ, WORK, INFO )
*
...
*     .. External Subroutines ..
      EXTERNAL           ZGEQRF, ZGGHRD, ZLACPY, ZLASET, ZUNGQR, ZUNMQR
*     ..
...
*
      IROWS = IHI + 1 - ILO
      ICOLS = N + 1 - ILO
      CALL ZGEQRF( IROWS, ICOLS, B( ILO, ILO ), LDB, WORK, Z, N*LDZ,
     $             IINFO )
      IF( IINFO.NE.0 ) THEN
         INFO = 2
         GO TO 10
...
*
     ...
     $                Q( ILO+1, ILO ), LDQ )
         CALL ZUNGQR( IROWS, IROWS, IROWS, Q( ILO, ILO ), LDQ, WORK, Z,
     $                N*LDZ, IINFO )
         IF( IINFO.NE.0 ) THEN
            INFO = 4
            GO TO 10
