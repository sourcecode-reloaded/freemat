/* ../../../dependencies/lapack/src/dstedc.f -- translated by f2c (version 20061008).
...
/*     use. */
    ...
    if (icompz == 2) {
	dlaset_("Full", n, n, &c_b18, &c_b19, &z__[z_offset], ldz, (ftnlen)4);
    }
...
/*     then multiply back into Z. */
		...
		dgemm_("N", "N", n, &m, &m, &c_b19, &work[storez], ldz, &work[
			1], &m, &c_b18, &z__[start * z_dim1 + 1], ldz, (
			ftnlen)1, (ftnlen)1);
	    } else if (icompz == 2) {
		dsteqr_("I", &m, &d__[start], &e[start], &z__[start + start * 
			z_dim1], ldz, &work[1], info, (ftnlen)1);
	    } else {
