/* ../../../dependencies/lapack/src/stgex2.f -- translated by f2c (version 20061008).
...
/*                          [ SCALE * identity(N2) ] */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    sscal_(n1, &c_b44, &li[(i__ << 2) - 4], &c__1);
...
/*        Set lower triangle of B-part to zero */
	...
	for (i__ = 2; i__ <= i__1; ++i__) {
	    i__2 = m - i__ + 1;
	    scopy_(&i__2, &c_b3, &c__0, &t[i__ + (i__ - 1 << 2) - 5], &c__1);
/* L50: */
	}
...
/*        transformations and set N1-by-N2 (2,1)-block to zero. */
	...
	i__1 = *n2;
	for (i__ = 1; i__ <= i__1; ++i__) {
	    scopy_(n1, &c_b3, &c__0, &s[*n2 + 1 + (i__ << 2) - 5], &c__1);
/* L60: */
	}
...
/*        Standardize existing 2-by-2 blocks. */
	...
	sgemm_("T", "N", n2, n1, n2, &c_b38, &work[1], &m, &a[*j1 + (*j1 + *
		n2) * a_dim1], lda, &c_b3, &work[m * m + 1], n2, (ftnlen)1, (
		ftnlen)1);
	slacpy_("Full", n2, n1, &work[m * m + 1], n2, &a[*j1 + (*j1 + *n2) * 
		a_dim1], lda, (ftnlen)4);
	sgemm_("T", "N", n2, n1, n2, &c_b38, &work[1], &m, &b[*j1 + (*j1 + *
...
/*        Accumulate transformations into Q and Z if requested. */
	...
	if (*wantq) {
	    sgemm_("N", "N", n, &m, &m, &c_b38, &q[*j1 * q_dim1 + 1], ldq, li,
	...
	if (*wantz) {
	    sgemm_("N", "N", n, &m, &m, &c_b38, &z__[*j1 * z_dim1 + 1], ldz, 
...
/*                (A(1:J1-1, J1:J1+M), B(1:J1-1, J1:J1+M)). */
	...
	i__ = *j1 - 1;
	if (i__ > 0) {
	    sgemm_("N", "N", &i__, &m, &m, &c_b38, &a[*j1 * a_dim1 + 1], lda, 
		    ir, &c__4, &c_b3, &work[1], &i__, (ftnlen)1, (ftnlen)1);
	    slacpy_("Full", &i__, &m, &work[1], &i__, &a[*j1 * a_dim1 + 1], 
		    lda, (ftnlen)4);
	    sgemm_("N", "N", &i__, &m, &m, &c_b38, &b[*j1 * b_dim1 + 1], ldb, 
		    ir, &c__4, &c_b3, &work[1], &i__, (ftnlen)1, (ftnlen)1);
