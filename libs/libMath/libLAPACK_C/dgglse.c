/* ../../../dependencies/lapack/src/dgglse.f -- translated by f2c (version 20061008).
	...
	integer *info)
{
    ...
    extern /* Subroutine */ int dgemv_(char *, integer *, integer *, 
	    ...
	    *, doublereal *, integer *, ftnlen, ftnlen, ftnlen), dtrsv_(char *
	    , char *, char *, integer *, doublereal *, integer *, doublereal *
	    , integer *, ftnlen, ftnlen, ftnlen), dggrqf_(integer *, integer *
...
/*     Solve T12*x2 = d for x2 */
    ...
    dtrsv_("Upper", "No transpose", "Non unit", p, &b[(*n - *p + 1) * b_dim1 
	    + 1], ldb, &d__[1], &c__1, (ftnlen)5, (ftnlen)12, (ftnlen)8);

/*     Update c1 */

    i__1 = *n - *p;
    dgemv_("No transpose", &i__1, p, &c_b29, &a[(*n - *p + 1) * a_dim1 + 1], 
	    lda, &d__[1], &c__1, &c_b31, &c__[1], &c__1, (ftnlen)12);

/*     Sovle R11*x1 = c1 for x1 */

    i__1 = *n - *p;
    dtrsv_("Upper", "No transpose", "Non unit", &i__1, &a[a_offset], lda, &
	    c__[1], &c__1, (ftnlen)5, (ftnlen)12, (ftnlen)8);
...
/*     Compute the residual vector: */
	...
	nr = *m + *p - *n;
	i__1 = *n - *m;
	dgemv_("No transpose", &nr, &i__1, &c_b29, &a[*n - *p + 1 + (*m + 1) *
		 a_dim1], lda, &d__[nr + 1], &c__1, &c_b31, &c__[*n - *p + 1],
		 &c__1, (ftnlen)12);
    } else {
	nr = *p;
    }
    dtrmv_("Upper", "No transpose", "Non unit", &nr, &a[*n - *p + 1 + (*n - *
	    p + 1) * a_dim1], lda, &d__[1], &c__1, (ftnlen)5, (ftnlen)12, (
	    ftnlen)8);
