/* ../../../dependencies/lapack/src/sgelss.f -- translated by f2c (version 20061008).
	...
	rank, real *work, integer *lwork, integer *info)
{
    ...
    static integer itau;
    static real vdum[1];
    static integer iascl, ibscl, chunk;
...
/*        Matrix all zero. Return zero solution. */
	...
	i__1 = max(*m,*n);
	slaset_("F", &i__1, nrhs, &c_b74, &c_b74, &b[b_offset], ldb, (ftnlen)
		1);
	slaset_("F", &minmn, &c__1, &c_b74, &c_b74, &s[1], &c__1, (ftnlen)1);
	*rank = 0;
	goto L70;
    }
...
/*        (Workspace: need BDSPAC) */
	...
	sbdsqr_("U", n, n, &c__0, nrhs, &s[1], &work[ie], &a[a_offset], lda, 
		vdum, &c__1, &b[b_offset], ldb, &work[iwork], info, (ftnlen)1)
		;
	if (*info != 0) {
	    goto L70;
	}
...
/*        (Workspace: need N, prefer N*NRHS) */
	...
	if (*lwork >= *ldb * *nrhs && *nrhs > 1) {
	    sgemm_("T", "N", n, nrhs, n, &c_b108, &a[a_offset], lda, &b[
		    b_offset], ldb, &c_b74, &work[1], ldb, (ftnlen)1, (ftnlen)
		    1);
...
/* Computing MIN */
		...
		sgemm_("T", "N", n, &bl, n, &c_b108, &a[a_offset], lda, &b[
			i__ * b_dim1 + 1], ldb, &c_b74, &work[1], n, (ftnlen)
			1, (ftnlen)1);
...
/*        (Workspace: need M*M+2*M, prefer M*M+M+M*NRHS) */
		...
		sgemm_("T", "N", m, nrhs, m, &c_b108, &work[il], &ldwork, &b[
			b_offset], ldb, &c_b74, &work[iwork], ldb, (ftnlen)1, 
			(ftnlen)1);
...
/* Computing MIN */
		    ...
		    sgemm_("T", "N", m, &bl, m, &c_b108, &work[il], &ldwork, &
			    b[i__ * b_dim1 + 1], ldb, &c_b74, &work[iwork], n,
			     (ftnlen)1, (ftnlen)1);
		    slacpy_("G", m, &bl, &work[iwork], n, &b[i__ * b_dim1 + 1]
			    , ldb, (ftnlen)1);
/* L40: */
		}
	    } else {
		sgemv_("T", m, m, &c_b108, &work[il], &ldwork, &b[b_dim1 + 1],
			 &c__1, &c_b74, &work[iwork], &c__1, (ftnlen)1);
...
/*        (Workspace: need N, prefer N*NRHS) */
		...
		sgemm_("T", "N", n, nrhs, m, &c_b108, &a[a_offset], lda, &b[
			b_offset], ldb, &c_b74, &work[1], ldb, (ftnlen)1, (
			ftnlen)1);
...
/* L60: */
		...
		sgemv_("T", m, n, &c_b108, &a[a_offset], lda, &b[b_offset], &
			c__1, &c_b74, &work[1], &c__1, (ftnlen)1);
		scopy_(n, &work[1], &c__1, &b[b_offset], &c__1);
	    }
	}
    }
