/* ../../../dependencies/lapack/src/dlar1v.f -- translated by f2c (version 20061008).
...
/* Computing MIN */
	...
	for (i__ = *r__ - 1; i__ >= i__1; --i__) {
	    ...
	    } else if ((d__1 = z__[i__ + 1], abs(d__1)) <= eps && (d__2 = z__[
		    i__ + 2], abs(d__2)) <= eps) {
		isuppz[1] = i__ + 3;
		goto L170;
	    } else {
...
L170:
	...
	for (i__ = *r__; i__ <= i__1; ++i__) {
	    ...
	    } else if ((d__1 = z__[i__], abs(d__1)) <= eps && (d__2 = z__[i__ 
		    - 1], abs(d__2)) <= eps) {
		isuppz[2] = i__ - 2;
		goto L190;
	    } else {
