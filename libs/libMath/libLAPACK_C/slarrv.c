/* ../../../dependencies/lapack/src/slarrv.f -- translated by f2c (version 20061008).
...
/* L20: */
    ...
    slaset_("Full", n, n, &c_b6, &c_b6, &z__[z_offset], ldz, (ftnlen)4);
    mgstol = eps * 5.f;
...
/*              (if necessary) to get approximate eigenvalues. */
		...
		if (ndepth > 0) {
		    ...
		    scopy_(&in, &z__[ibegin + j * z_dim1], &c__1, &d__[ibegin]
			    , &c__1);
		    scopy_(&in, &z__[ibegin + (j + 1) * z_dim1], &c__1, &l[
			    ibegin], &c__1);
		    sigma = l[iend];
		}
		k = ibegin;
...
/*                    continue (to the next loop) */
			...
			if (! mgscls) {
			    slarrf_(&in, &d__[ibegin], &l[ibegin], &work[
				    ...
				    newlst, &work[1], &z__[ibegin + newftt * 
				    z_dim1], &z__[ibegin + (newftt + 1) * 
				    z_dim1], &work[indwrk], &iwork[iindwk], 
				    info);
			    if (*info == 0) {
				++nclus;
				k = newcls + (nclus << 1);
...
/* Computing MIN */
			    ...
			    sscal_(&in, &nrminv, &z__[ibegin + ktot * z_dim1],
				     &c__1);
			    ++ktot;
/* L100: */
			}
			if (newsiz > 1) {
				...
				for (q = newftt; q <= i__5; ++q) {
				    tmp1 = -sdot_(&in, &z__[ibegin + p * 
					    z_dim1], &c__1, &z__[ibegin + q * 
					    z_dim1], &c__1);
				    saxpy_(&in, &tmp1, &z__[ibegin + q * 
					    z_dim1], &c__1, &z__[ibegin + p * 
					    z_dim1], &c__1);
/* L110: */
				}
				tmp1 = 1.f / snrm2_(&in, &z__[ibegin + p * 
					z_dim1], &c__1);
				sscal_(&in, &tmp1, &z__[ibegin + p * z_dim1], 
					&c__1);
/* Computing MIN */
