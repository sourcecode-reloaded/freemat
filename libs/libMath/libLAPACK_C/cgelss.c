/* ../../../dependencies/lapack/src/cgelss.f -- translated by f2c (version 20061008).
...
/*     Test the input arguments */
    ...
    *info = 0;
    minmn = min(*m,*n);
    maxmn = max(*m,*n);
    mnthr = ilaenv_(&c__6, "CGELSS", " ", m, n, nrhs, &c_n1, (ftnlen)6, (
	    ftnlen)1);
    lquery = *lwork == -1;
    if (*m < 0) {
	*info = -1;
...
/* Computing MAX */
	    i__1 = maxwrk, i__2 = *n + *nrhs * ilaenv_(&c__1, "CUNMQR", "LC", 
...
/* Computing MAX */
		i__1 = maxwrk, i__2 = *m * 3 + *m * *m + (*m << 1) * ilaenv_(&
			c__1, "CGEBRD", " ", m, m, &c_n1, &c_n1, (ftnlen)6, (
			ftnlen)1);
		maxwrk = max(i__1,i__2);
/* Computing MAX */
...
/* Computing MAX */
		i__1 = maxwrk, i__2 = *m * 3 + *m * *m + (*m - 1) * ilaenv_(&
			c__1, "CUNGBR", "P", m, m, m, &c_n1, (ftnlen)6, (
			ftnlen)1);
		maxwrk = max(i__1,i__2);
		if (*nrhs > 1) {
/* Computing MAX */
...
/*        Matrix all zero. Return zero solution. */
	...
	slaset_("F", &minmn, &c__1, &c_b78, &c_b78, &s[1], &minmn, (ftnlen)1);
	*rank = 0;
	goto L70;
    }
...
/*        (RWorkspace: need BDSPAC) */
	...
	cbdsqr_("U", n, n, &c__0, nrhs, &s[1], &rwork[ie], &a[a_offset], lda, 
		vdum, &c__1, &b[b_offset], ldb, &rwork[irwork], info, (ftnlen)
		1);
	if (*info != 0) {
	    goto L70;
	}
...
/* Computing MIN */
		    ...
		    cgemm_("C", "N", m, &bl, m, &c_b2, &work[il], &ldwork, &b[
			    ...
			    ftnlen)1, (ftnlen)1);
		    clacpy_("G", m, &bl, &work[iwork], n, &b[i__ * b_dim1 + 1]
			    , ldb, (ftnlen)1);
/* L40: */
		}
	    } else {
