/* ../../../dependencies/lapack/src/dlarrv.f -- translated by f2c (version 20061008).
...
/* L20: */
    ...
    dlaset_("Full", n, n, &c_b6, &c_b6, &z__[z_offset], ldz, (ftnlen)4);
    mgstol = eps * 5.;
...
/*              (if necessary) to get approximate eigenvalues. */
		...
		if (ndepth > 0) {
		    ...
		    dcopy_(&in, &z__[ibegin + j * z_dim1], &c__1, &d__[ibegin]
			    , &c__1);
		    dcopy_(&in, &z__[ibegin + (j + 1) * z_dim1], &c__1, &l[
			    ibegin], &c__1);
		    sigma = l[iend];
		}
		k = ibegin;
...
/* Computing MIN */
			    ...
			    dscal_(&in, &nrminv, &z__[ibegin + ktot * z_dim1],
				     &c__1);
			    ++ktot;
/* L100: */
			}
			if (newsiz > 1) {
				...
				for (q = newftt; q <= i__5; ++q) {
				    tmp1 = -ddot_(&in, &z__[ibegin + p * 
					    z_dim1], &c__1, &z__[ibegin + q * 
					    z_dim1], &c__1);
				    daxpy_(&in, &tmp1, &z__[ibegin + q * 
					    z_dim1], &c__1, &z__[ibegin + p * 
					    z_dim1], &c__1);
/* L110: */
				}
				tmp1 = 1. / dnrm2_(&in, &z__[ibegin + p * 
					z_dim1], &c__1);
				dscal_(&in, &tmp1, &z__[ibegin + p * z_dim1], 
					&c__1);
/* Computing MIN */
