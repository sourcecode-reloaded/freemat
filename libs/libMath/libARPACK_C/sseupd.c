/* ../SRC/sseupd.f -- translated by f2c (version 20061008).
	...
	ftnlen bmat_len, ftnlen which_len)
{
    ...
    static real kv[2];
    static integer ibd, ihb, ihd, ldh, ilg, ldq, ism, irz, mode;
    static real eps23;
...
/*        %------------------------------------------------% */
	...
	if (s_cmp(which, "LM", (ftnlen)2, (ftnlen)2) == 0 || s_cmp(which, 
	    ...
	    if (msglvl > 2) {
		svout_(&debug_1.logfil, &c__1, &thres1, &debug_1.ndigit, 
...
/*            %------------------------------------------------% */
		...
		kv[0] = thres1;
		kv[1] = thres2;
		svout_(&debug_1.logfil, &c__2, kv, &debug_1.ndigit, "_seupd:"
			" Threshold eigenvalues used for re-ordering", (ftnlen)
			50);
	    }
...
/*        %----------------------------------------------------------% */
	...
	for (j = 0; j <= i__1; ++j) {
		...
		if ((r__1 = workl[irz + j], dabs(r__1)) >= dabs(thres1)) {
/* Computing MAX */
		    r__2 = eps23, r__3 = (r__1 = workl[irz + j], dabs(r__1));
		    tempbnd = dmax(r__2,r__3);
...
/*        %-------------------------------------------% */
	...
	if (msglvl > 2) {
	    ivout_(&debug_1.logfil, &c__1, &ktrord, &debug_1.ndigit, "_seupd"
...
/* L120: */
    ...
    if (s_cmp(type__, "REGULR", (ftnlen)6, (ftnlen)6) != 0) {
	sger_(n, &nconv, &c_b119, &resid[1], &c__1, &workl[iw], &c__1, &z__[
		z_offset], ldz);
    }
