/* ../SRC/sneupd.f -- translated by f2c (version 20061008).
	...
	which_len)
{
    ...
    static integer iheigi, iheigr, bounds, invsub, iuptri, ktrord, msglvl, 
	    outncv;
...
/*        %-------------------------------------------% */
	...
	if (s_cmp(which, "LM", (ftnlen)2, (ftnlen)2) == 0 || s_cmp(which, 
	    ...
	    thres = slapy2_(&workl[ritzr], &workl[ritzi]);
	    ...
	    thres = (r__1 = workl[ritzi], dabs(r__1));
	}

	if (msglvl > 2) {
	    svout_(&debug_1.logfil, &c__1, &thres, &debug_1.ndigit, "_neupd:"
...
/*        %----------------------------------------------------------% */
	...
	for (j = 0; j <= i__1; ++j) {
		...
		if (slapy2_(&workl[irr + j], &workl[iri + j]) >= thres) {
/* Computing MAX */
		    r__1 = eps23, r__2 = slapy2_(&workl[irr + j], &workl[iri 
			    + j]);
...
/* Computing MAX */
		...
		if ((r__1 = workl[iri + j], dabs(r__1)) >= thres) {
/* Computing MAX */
		    r__1 = eps23, r__2 = slapy2_(&workl[irr + j], &workl[iri 
			    + j]);
		    temp1 = dmax(r__1,r__2);
...
/*        %-----------------------------------------------------------% */
	...
	slaset_("All", ncv, ncv, &c_b44, &c_b45, &workl[invsub], &ldq, (
		ftnlen)3);
...
/*           %---------------------------------------------------% */
		...
		sscal_(&nconv, &c_b71, &workl[iuptri + j - 1], &ldq);
		sscal_(&nconv, &c_b71, &workl[iuptri + (j - 1) * ldq], &c__1);
	    }
...
/* L40: */
	    ...
	    sgemv_("T", ncv, &nconv, &c_b45, &workl[invsub], &ldq, &workl[
		    ihbds], &c__1, &c_b44, &workev[1], &c__1, (ftnlen)1);
...
/*           %----------------------------------------------% */
	    ...
	    strmm_("Right", "Upper", "No transpose", "Non-unit", n, &nconv, &
		    c_b45, &workl[invsub], &ldq, &z__[z_offset], ldz, (ftnlen)
...
/*        %---------------------------------------% */
	...
	sger_(n, &nconv, &c_b45, &resid[1], &c__1, &workev[1], &c__1, &z__[
		z_offset], ldz);
