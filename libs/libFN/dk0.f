      SUBROUTINE DCALCK0(ARG,RESULT,JINT)
C--------------------------------------------------------------------
C
C This packet computes modified Bessel functions of the second kind
C   and order zero, K0(X) and EXP(X)*K0(X), for real
C   arguments X.  It contains two function type subprograms, DBESK0
C   and BESEK0, and one subroutine type subprogram, DCALCK0.
...
C   EXP(X)*K0(X), respectively.  The routine DCALCK0 is
...
C   subprograms invoke DCALCK0 with the statement
C          CALL DCALCK0(ARG,RESULT,JINT)
C   where the parameter usage is as follows
C
C      Function                     Parameters for DCALCK0
C       Call              ARG                  RESULT          JINT
C
C     DBESK0(ARG)   0 .LT. ARG .LE. XMAX       K0(ARG)           1
...
C   XSMALL = Argument below which DBESK0 and BESEK0 may
...
C            beta**maxexp
C   XMAX   = Largest argument acceptable to DBESK0;  Solution to
C            equation: 
...
C  DBESK0 entry returns the value 0.0 for ARG .GT. XMAX.
C
C
...
C--------------------------------------------------------------------
      JINT = 1
      CALL DCALCK0(X,RESULT,JINT)
      DBESK0 = RESULT
      RETURN
C---------- Last line of DBESK 0 ----------
      END
      DOUBLE PRECISION 
     1    FUNCTION DBESEK0(X)
C--------------------------------------------------------------------
C
C This function program computes approximate values for the
C   modified Bessel function of the second kind of order zero
