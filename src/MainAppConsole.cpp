...
// void MainAppConsole::UpdateTermWidth(int w) {
//   m_eval->setTerminalWidth(w);
// }

void MainAppConsole::ExecuteLine(QString txt) {
...
//
//void MainAppConsole::CheckNonClosable() {
//  if (GUIHack && !wid) {
//    wid = new NonClosable;
...
//}
//
void MainAppConsole::DoGraphicsCall(Interpreter* interp, FuncPtr f, ArrayVector m, int narg) { 
...
void MainAppConsole::outputRawText(QString s)
{
  std::cout << qPrintable(s);
}

void MainAppConsole::EnableRepaint() {
  //  GfxEnableRepaint();
}

void MainAppConsole::DisableRepaint() {
  //  GfxDisableRepaint();
}
...
void LoadBuiltinFunctions(Context *context, bool guimode)
{
  ...
  LoadBuiltinFunctionsCore(context,true);
...
Context *MainAppConsole::NewContext() {
  Context *context = new Context(m_global);
  LoadBuiltinFunctions(context,false);
  if (first_time) {
    first_time = false;
...
void MainAppConsole::UpdateBasePath(QString rootpath) {
...
void MainAppConsole::SetRootPath(QString path, Interpreter* eval) {
  if (inBundleMode()) 
...
void MainAppConsole::UpdatePaths() {
...
int MainAppConsole::StartNewInterpreterThread() {
  Interpreter *p_eval = new Interpreter(NewContext());
  p_eval->setBasePath(basePath);
  p_eval->setUserPath(userPath);
  connect(p_eval,SIGNAL(outputRawText(QString)),this,SLOT(outputRawText(QString)));
...
//
void MainAppConsole::CollectProfileSample() {
  unsigned linenumber;
...
void MainAppConsole::EnableProfiler( bool bEnable )
{
    emit SetEnable( bEnable );
}

void MainAppConsole::ControlProfiler(bool enableflag) {
  if (enableflag)
    profilerTimer->start();
  else
    profilerTimer->stop();
...
void MainAppConsole::RegisterInterrupt() {
...
//void MainAppConsole::RefreshFigures() {
  //  RefreshFigs();
//}

void MainAppConsole::DoStuff() {
  m_eval->ExecuteLine("demo\r\n");
}

int MainAppConsole::Run() {
  UpdatePaths();
  ...
  connect(refreshTimer,SIGNAL(timeout()),this,SLOT(DoStuff()));
  refreshTimer->start();
