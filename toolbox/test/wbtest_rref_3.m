function test_val = wbtest_rref_3(verbose)
  ...
  y2 = [];
    ...
    if (error_flag && ~error_refs(loopi) && ~((any(loopi==[15,62]))))
       printf('Mismatch Errors: input %d [y1,y2]=rref(x1,.0001)\n',loopi);
       fail_count = fail_count + 1;
    elseif (~error_flag && ~error_refs(loopi) && ~wbtest_near(y1,y1_refs{loopi}) && ~((any(loopi==[15,62]))))
      printf('Mismatch (near): input %d output 1 [y1,y2]=rref(x1,.0001)\n',loopi);
      fail_count = fail_count + 1;
    elseif (~error_flag && ~error_refs(loopi) && ~wbtest_near(y2,y2_refs{loopi}) && ~((any(loopi==[15,62]))))
