function test_val = wbtest_rightdivide_1(verbose)
  ...
  for loopi=1:numel(wbinputs)
    ...
    if (error_flag && ~error_refs(loopi,loopj) && ~(((size(x1,1)==1)||any(isinf(x1))||any(isinf(x2))||(loopi==64)||(loopj==64))))
       printf('Mismatch Errors: input %d, %d y1=x1/x2\n',loopi,loopj);
        fail_count = fail_count + 1;
  elseif (~error_flag && ~error_refs(loopi,loopj) && ~wbtest_exact(y1,y1_refs{loopi,loopj}) && ~(((size(x1,1)==1)||any(isinf(x1))||any(isinf(x2))||(loopi==64)||(loopj==64))))
